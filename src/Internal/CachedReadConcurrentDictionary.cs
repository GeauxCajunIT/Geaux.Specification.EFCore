...
namespace Geaux.Specification.EfCore.Internal;
...
internal class CachedReadConcurrentDictionary<TKey, TValue> : IDictionary<TKey, TValue> where TKey : notnull
{
    ...
    {
        bool result = ((IDictionary<TKey, TValue>)_dictionary).Remove(item);
    ...
    {
        bool result = ((IDictionary<TKey, TValue>)_dictionary).Remove(key);
        if (result) InvalidateCache();
        return result;
    }
